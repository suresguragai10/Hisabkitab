...
| 4.8 | Bilingual support incomplete | 🟡 Partial | Confirmed 2026-07-26: only the sidebar nav and `VoucherEntry.jsx` (12 strings) actually use `src/lib/i18n.js`'s `t()` — every other page is 100% hardcoded English. User chose to prioritize client-facing print documents first. Fixed: Invoice/Bill/Credit-Debit-Note print templates now have consistent dual-language labels throughout (table headers, meta fields, totals, footer) — previously only the document title had Nepali, everything else was English-only. Not done by a certified translator; standard low-ambiguity business vocabulary, worth a native-speaker check. Amount-in-words stays English-only (Nepali numeral-to-words is separate, larger work). **Still open**: the rest of the app (Dashboard, Reports, Settings, TDS, VAT Filing, Sales Orders/RFQ, Chart of Accounts, Contacts, Items, Inventory, Team, Audit Log, Bank Reconciliation — everything except the sidebar/VoucherEntry) remains English-only. |
